...
import { MapPin, Phone, CheckCircle, Navigation, Package } from '@/lib/icons';
...
export default function RiderDashboard() {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-20 space-y-4">
            {/* Stats */}
            ...
            {tasks.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
                    <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-teal-50 text-teal-600 flex items-center justify-center">
                        <Package className="w-6 h-6" />
                    </div>
                    <p className="text-gray-900 font-bold mb-1">No deliveries assigned</p>
                    <p className="text-sm text-gray-500">New tasks will appear here when they are assigned.</p>
                </div>
            ) : (
                tasks.map((task) => (
                    <div key={task.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="p-4 border-b border-gray-50 flex justify-between items-start">
                            <div>
                                ...
                                <div className="text-xs text-gray-500 mt-1">{task.items} Items â€¢ â‚¹{task.amount}</div>
                            </div>
