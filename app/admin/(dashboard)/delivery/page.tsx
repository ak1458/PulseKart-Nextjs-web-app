...
import { Truck, MapPin, Plus, Edit, Trash2, X } from '@/lib/icons';
...
export default function AdminDeliveryPage() {
    ...
    const fetchZones = async () => {
        try {
            ...
            console.log('API not available, using empty zones');
            setZones([]);
        }
    };
        ...
        <div className="space-y-6 animate-fade-in pb-20 md:pb-0">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-400 text-sm">Manage delivery zones and shipping rates</p>
                </div>
                <button
                    onClick={() => {
                        setEditingZone(null);
            ...
            {zones.length === 0 ? (
                <div className="glass-panel p-12 rounded-2xl border border-white/10 text-center flex flex-col items-center justify-center">
                    ...
                    <h2 className="text-xl font-bold text-white mb-2">No Delivery Zones</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {zones.map((zone) => (
                        <div key={zone.id} className="glass-panel p-6 rounded-2xl border border-white/10 relative group hover:border-teal-500/30 transition-all">
                            ...
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                <button onClick={() => handleEdit(zone)} className="text-gray-400 hover:text-teal-400"><Edit className="w-4 h-4" /></button>
                                <button onClick={() => handleDelete(zone.id)} className="text-gray-400 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                            </div>
                            ...
                            <div className="space-y-2 text-sm text-gray-400 relative z-10">
                                <div className="flex justify-between border-b border-white/5 pb-2">
                                    ...
                                    <span className="font-bold text-white">₹{zone.standard_cost}</span>
                                </div>
                                <div className="flex justify-between border-b border-white/5 pb-2">
                                    <span>Express Shipping:</span>
                                    <span className="font-bold text-white">₹{zone.express_cost}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Est. Delivery:</span>
                                    <span className="font-medium text-gray-300">{zone.delivery_days_min}-{zone.delivery_days_max} Days</span>
                                </div>
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        ...
                        >
                            <div className="p-6 border-b border-white/10 flex justify-between items-center">
                                <h3 className="text-lg font-bold text-white">{editingZone ? 'Edit Zone' : 'Add New Zone'}</h3>
                                <button onClick={() => setIsModalOpen(false)}><X className="w-5 h-5 text-gray-400 hover:text-white" /></button>
                            </div>
                            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-400 mb-1">Zone Name</label>
                                    <input
                                        type="text"
                                        required
                                        value={formData.name}
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-400 mb-1">Pincodes (Comma separated)</label>
                                    <textarea
                                        rows={3}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-400 mb-1">Standard Cost (₹)</label>
                                        <input
                                            type="number"
                                            required
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-400 mb-1">Express Cost (₹)</label>
                                        <input
                                            type="number"
                                            required
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-400 mb-1">Min Days</label>
                                        <input
                                            type="number"
                                            required
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-gray-400 mb-1">Max Days</label>
                                        <input
                                            type="number"
                                            required
