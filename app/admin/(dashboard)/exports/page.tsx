...
export default function ExportHistoryPage() {
    ...
    useEffect(() => {
        const savedJobs = localStorage.getItem('export_jobs');
        if (savedJobs) {
    ...
    const handleCreateExport = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `export-${formData.job_type}-${Date.now()}.xlsx`;
            ...
            setJobs(updatedJobs);
            localStorage.setItem('export_jobs', JSON.stringify(updatedJobs));
    ...
    const handleDelete = (id: string) => {
        if (confirm('Are you sure you want to remove this export record?')) {
                        ...
                        {jobs.map((job) => (
                            <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right">
                                    {job.status === 'complete' && (
                                        <a href={job.output_url} className="inline-flex items-center gap-1 text-teal-600 font-bold hover:underline mr-3">
