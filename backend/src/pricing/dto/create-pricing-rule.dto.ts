  ...
  .refine(
    (data) => {
      ...
      switch (data.type) {
        ...
        case 'tiered_pricing':
          return (
            data.actions.tiers !== undefined && data.actions.tiers.length > 0
          );
        default:
          return true;
      }
    ...
    {
      message:
        'Actions must contain the required fields for the selected rule type',
      path: ['actions'],
    },
  );

export class CreatePricingRuleDto extends createZodDto(
