...
export class PricingService {
  ...
  async update(
    ...
    if (updateDto.startDate !== undefined) {
      rule.startDate = updateDto.startDate
    ...
    if (updateDto.conditions !== undefined)
      rule.conditions = updateDto.conditions;
  ...
  evaluateConditions(
    ...
    if (
      conditions.minQuantity !== undefined &&
      quantity < conditions.minQuantity
    ) {
      return false;
    }
    if (
      conditions.maxQuantity !== undefined &&
      quantity > conditions.maxQuantity
    ) {
      return false;
    }
    ...
    // Check product ID
    if (
      conditions.productIds !== undefined &&
    ...
    // Check category ID
    if (
      conditions.categoryIds !== undefined &&
      conditions.categoryIds.length > 0
    ) {
      if (!categoryId || !conditions.categoryIds.includes(categoryId))
        return false;
    }
    ...
    if (conditions.userTags !== undefined && conditions.userTags.length > 0) {
      if (
        !userTags ||
        !conditions.userTags.some((tag) => userTags.includes(tag))
      ) {
        return false;
      }
    }
