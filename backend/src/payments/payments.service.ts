...
import {
  ...
  DiscountType,
} from './entities/payment-gateway-discount.entity';
import {
  PaymentTransaction,
  PaymentStatus,
  PaymentMetadata,
...
export class PaymentsService {
  ...
  async updatePaymentMethod(
    ...
    Object.assign(paymentMethod, {
      ...
      config: updateDto.config
        ? { ...paymentMethod.config, ...updateDto.config }
        : paymentMethod.config,
      supportedCurrencies:
        updateDto.supportedCurrencies ?? paymentMethod.supportedCurrencies,
      ...
      processingFeePercent:
        updateDto.processingFeePercent ?? paymentMethod.processingFeePercent,
  ...
  async createGatewayDiscount(
    createDto: CreateGatewayDiscountDto,
  ): Promise<PaymentGatewayDiscount> {
  ...
  async findGatewayDiscountById(id: string): Promise<PaymentGatewayDiscount> {
    const discount = await this.gatewayDiscountRepository.findOne({
      where: { id },
    });
    if (!discount) {
  ...
  async findDiscountsByPaymentMethod(
    code: PaymentMethodCode,
  ): Promise<PaymentGatewayDiscount[]> {
    return this.gatewayDiscountRepository.find({
  ...
  async updateGatewayDiscount(
    id: string,
    updateDto: UpdateGatewayDiscountDto,
  ): Promise<PaymentGatewayDiscount> {
    ...
    Object.assign(discount, {
      ...
      maxDiscountAmount:
        updateDto.maxDiscountAmount ?? discount.maxDiscountAmount,
  ...
  async getActivePaymentMethods(
    currency: string = 'INR',
  ): Promise<ActivePaymentMethodResponse[]> {
  ...
  async calculatePaymentDiscount(
    ...
    const discount = await this.getApplicableDiscount(
      paymentMethodCode,
      orderAmount,
    );
  ...
  async processPayment(
    ...
    const { orderId, paymentMethodCode, amount, currency, metadata } =
      processDto;
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to create payment: ${savedTransaction.errorMessage}`,
      );
    }
  }
  ...
  async verifyPayment(
    ...
    try {
      ...
      if (result.success) {
        ...
        await this.incrementDiscountUsage(
          transaction.paymentMethodCode,
          transaction.amount,
        );
      } else {
      ...
      throw new BadRequestException(
        `Payment verification failed: ${transaction.errorMessage}`,
      );
    }
  }
  ...
  async getTransactions(
    ...
    const queryBuilder =
      this.paymentTransactionRepository.createQueryBuilder('transaction');
    ...
    if (filters.paymentMethodCode) {
      queryBuilder.andWhere('transaction.paymentMethodCode = :code', {
        code: filters.paymentMethodCode,
      });
    }
