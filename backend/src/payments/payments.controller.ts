...
export class PaymentsController {
  ...
  async getActivePaymentMethods(@Query('currency') currency?: string) {
    const methods = await this.paymentsService.getActivePaymentMethods(
      currency || 'INR',
    );
    return {
      success: true,
  ...
  async checkDiscount(
    ...
    const discount = await this.paymentsService.calculatePaymentDiscount(
      code,
      orderAmount,
    );
    return {
...
export class AdminPaymentsController {
  ...
  async updatePaymentMethod(
    ...
    const method = await this.paymentsService.updatePaymentMethod(
      id,
      updateDto,
    );
    return {
  ...
  async createGatewayDiscount(
    @Body(new ZodValidationPipe(createGatewayDiscountSchema))
    createDto: CreateGatewayDiscountDto,
  ) {
    const discount =
      await this.paymentsService.createGatewayDiscount(createDto);
    return {
      success: true,
      data: discount,
  ...
  async updateGatewayDiscount(
    @Param('id') id: string,
    @Body(new ZodValidationPipe(updateGatewayDiscountSchema))
    updateDto: UpdateGatewayDiscountDto,
  ) {
    const discount = await this.paymentsService.updateGatewayDiscount(
      id,
      updateDto,
    );
    return {
