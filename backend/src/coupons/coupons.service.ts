...
export class CouponsService {
  ...
  async create(createCouponDto: CreateCouponDto): Promise<Coupon> {
    ...
    coupon.start_date = createCouponDto.start_date
      ? new Date(createCouponDto.start_date)
      : null;
    coupon.end_date = createCouponDto.end_date
  ...
  async update(
    ...
    if (
      ...
      updateCouponDto.code.toUpperCase() !== coupon.code
    ) {
      const existingCoupon = await this.couponRepository.findOne({
    ...
    if (updateCouponDto.start_date !== undefined) {
      coupon.start_date = updateCouponDto.start_date
        ? new Date(updateCouponDto.start_date)
        : null;
    }
    if (updateCouponDto.end_date !== undefined) {
      coupon.end_date = updateCouponDto.end_date
        ? new Date(updateCouponDto.end_date)
        : null;
    }
  ...
  async getActiveCoupons(): Promise<CouponListResponse[]> {
    ...
    const coupons = await this.couponRepository
      ...
      .andWhere('(coupon.start_date IS NULL OR coupon.start_date <= :now)', {
        now,
      })
      .andWhere('(coupon.end_date IS NULL OR coupon.end_date >= :now)', { now })
      .andWhere(
