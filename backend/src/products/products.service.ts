...
export class ProductsService {
    ...
    async findAll(limit: number = 50, offset: number = 0, filters?: FilterOptions): Promise<ProductWithStock[]> {
        ...
        if (filters?.minPrice !== undefined && !isNaN(filters.minPrice)) {
            queryBuilder.andWhere('p.price >= :minPrice', { minPrice: filters.minPrice });
        }

        if (filters?.maxPrice !== undefined && !isNaN(filters.maxPrice)) {
            queryBuilder.andWhere('p.price <= :maxPrice', { maxPrice: filters.maxPrice });
        }
