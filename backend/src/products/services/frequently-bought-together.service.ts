...
export class FrequentlyBoughtTogetherService {
    ...
    async create(createFbtDto: CreateFbtDto): Promise<FrequentlyBoughtTogether> {
        ...
        if (createFbtDto.primaryProductId === createFbtDto.relatedProductId) {
            throw new BadRequestException('Primary and related products must be different');
        }
    ...
    async update(id: number, updateFbtDto: UpdateFbtDto): Promise<FrequentlyBoughtTogether> {
    ...
    async bulkCreate(createFbtDtos: CreateFbtDto[]): Promise<FrequentlyBoughtTogether[]> {
